...
//		}
	 ...
	 * @return
	 */
	public static String getRepeatTypeEvent(CalendarEvent c, String day) {

//		c, c.getRepeatType(), dateString, c.getStartDay()
		String jsonstr = c.getRepeatType();
		String startDay =c.getStartDay();
...
//		System.out.println("beginDay.compareTo(day)  =  " + beginDay.compareTo(day));
		...
		if(c.getStartDay().equals(c.getEndDay())){
			c.setStartDay(beginDay);
			c.setEndDay(beginDay);
		} else {
			int inteval = comparePastDate(c.getEndDay(),c.getStartDay());
			if(0==inteval){
				c.setStartDay(beginDay);
				c.setEndDay(beginDay);
			} else {
				c.setStartDay(beginDay);
				c.setEndDay(newEndDate(c.getStartDay(),inteval));
			}
		}
	...
	private static String newEndDate(String startDay, int inteval) {
        String temp = "";
	...
	public static int comparePastDate(String oldDate,String nowDate) {
		try {
	        ...
	        return (int)((nTime - oTime)/(3600F * 1000 * 24));
		} catch(Exception e) {
			e.printStackTrace();
			return 0;
		}
