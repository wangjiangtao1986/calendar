...
Ext.define('Ext.ux.calendar.editor.DetailEditor', {
	...
	initComponent : function() {
		...
		this.exceptionField = this.exceptionField
				|| Ext.create('Ext.form.TextField', {
							fieldLabel : lan['exceptionField.label'],
							anchor : '100%', maxLength : 500 
						});
...
//			exceptionField
			...
			var key = "";
			for(var i in rt.exceptions) {
				key +=i+",";
			}
			if(""!=key){
				key = key.substr(0,key.length-1);
				this.exceptionField.setValue(key);
				items.get(6).show();
				items.get(6).setReadOnly(true)
			}
